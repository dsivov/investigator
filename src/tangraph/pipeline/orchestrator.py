...
The class is thin: it parses the request, branches on
``use_regular_triangulation``, and runs the seven-step standard pipeline
...
from crewai_mvp.src.crewai_mvp.tangos_generic_specialist import TangosGenericSpecialist
from crewai_mvp.src.crewai_mvp.tools.triangulation import Triangulation
from model2vec import StaticModel
from wordllama import WordLlama

from tangraph.graph import (
...
from tangraph.pipeline.response_builder import build_network_analysis_payload
from tangraph.pipeline.hypothesis import return_hypothesis_for_domain
from tangraph.state import EdgeRecord, EntityRecord, InvestigationState
from tangraph.utils import find_value_in_nested_dict, flatten_and_clean_dict, remove_empty_fields
...
# process; default ON. Set TANGRAPH_DISABLE_CACHE=1 to force every LLM call to
...
_cache_on = os.environ.get("TANGRAPH_DISABLE_CACHE") != "1"
...
async def extract_entities_from_chunk(json_chunk, original_chunk, working_state, task_id=None, investigation_query=""):
    ...
    try:
        ...
        tangos_log.error(f"Error during entity extraction from chunk: {e}")
        return [], [], []
...
async def get_representative_identifiers_task(identifiers: list[list[str]]) -> list[str]:
    ...
    for batch in grouped:
        if len(batch) > 1:
            ...
            tangos_log.debug("Single identifier in group, adding directly to representative identifiers")
...
async def named_entities_extractor_task(investigation_id: str, text: str, working_state, investigation_query: str = ""):
    ...
    tangos_log.info(f"Number of original chunks: {len(json_chunks)}")
    if len(json_chunks) == 0:
        tangos_log.warning("No chunks to process for entity extraction")
...
class InvestigationPipeline:
    ...
    async def run(self, payload: dict) -> dict:
        ...
        try:
            ...
            use_regular_triangulation = payload.get("use_regular_triangulation", False)
            ...
            if use_regular_triangulation:
                return await self._regular_triangulation(
                    investigation_id, domain, text, investigation_subject
                )
            ...
            tangos_log.error(f"Error during graph nodes extraction: {str(e)}")
            tangos_log.error(traceback.format_exc())
    ...
    async def _regular_triangulation(
        self,
        ...
        tangos_log.info("Using regular triangulation specialist")
        expert = self.session_store.get(investigation_id)
        if not expert:
            tangos_log.info(f"Creating new TangosGenericSpecialist for investigation id: {investigation_id}")
            expert = TangosGenericSpecialist(
                version="v2",
                output_folder="/tmp/debug_out",
                log_name_in=f"debug_log_{investigation_id}",
                data_retrieval={"serverless": True},
                domain="general",
            )
            self.session_store.set(investigation_id, expert)
        expert.domain = domain
        text = json.loads(text)
        triangulator = Triangulation(
            text,
            expert.entities,
            log_filename=expert.log_name,
            main_hypothesis=investigation_subject,
            domain=expert.domain,
        )
        await expert.tangraph_update_from_server(triangulator)
        merged_entities = [idn for idn in expert.entities if idn.get("type") == "entity" and idn.get("identifier")]
        edges_enrichment_results = [idn for idn in expert.entities if idn.get("type") == "affiliation"]
        self.session_store.set(investigation_id, expert)
        tangos_log.info("Regular triangulation process completed")
        return {
            "status": "success",
            "session_id": str(investigation_id),
            "nodes": merged_entities,
            "edges": edges_enrichment_results,
        }
    ...
    async def _standard_pipeline(
        ...
        *,
        ...
        if is_first_run:
            tangos_log.info(f"Starting new investigation session: {investigation_id}")
        else:
            tangos_log.info(f"Resuming investigation session: {investigation_id}")

        # --- Preprocess input ----------------------------------------------
        is_json_input = is_json(text)
        if is_json_input:
            tangos_log.debug("Input text is JSON, preprocessing")
            text = preprocess_text(text)
        else:
            if is_first_run:
                tangos_log.info("Input text is plain text")
                return {"status": "error", "message": "Input text must be in JSON format"}
            text = '{"data": "Previous investigation state, no new text provided."}'

        if not text:
            tangos_log.warning("No text provided, returning current state")
            return self._return_current_state(state)

        # --- Step 1: Named entities ----------------------------------------
        tangos_log.info("Starting named entities extraction process")
        ...
        if not is_input:
            tangos_log.warning("No entities extracted from input, returning current state")
            return self._return_current_state(state)
        ...
        if os.environ.get("TANGRAPH_SKIP_MRI"):
            ...
            tangos_log.info(
                f"TANGRAPH_SKIP_MRI=1 -- skipping MostRepresentativeIdentifier; "
        ...
        tangos_log.debug(f"Representative identifiers: {json.dumps(representative_identifiers, indent=4)}")
        ...
        # paraphrase patterns.
        tangos_log.info("Starting duplicate detection and merging process")
        ...
        if n_fixed:
            tangos_log.info(f"Post-hoc canonical-validation rewrote {n_fixed} headline-shaped entity identifiers")
        ...
        if not merged_entities:
            tangos_log.warning("No deduplicated nodes found after duplicate detection, returning current state")
            return self._return_current_state(state)
        ...
        tangos_log.info("Starting graph building process")
        ...
        if coarse_grained_edges and len(coarse_grained_edges) > 0:
            tangos_log.info(f"Coarse grained edges extracted: {len(coarse_grained_edges)}")
            edges_grouped_by_chunk = group_edges_by_chunk(coarse_grained_edges)
            tangos_log.info("Starting edges enrichment process")
            ...
            tangos_log.info("No coarse grained edges extracted, skipping edges enrichment process")
        ...
        tangraph = fully_connected_graph.copy(as_view=False)
        ...
        root = match_query_to_entity(
            investigation_query, merged_entities, representative_identifiers, tangraph,
        ...
        if not root and investigation_query:
            try:
                ...
                subject = None
                tangos_log.warning(f"Query-subject extraction failed ({type(e).__name__}); using most-connected root")
            if subject:
                root = match_query_to_entity(subject, merged_entities, representative_identifiers, tangraph)
                if root:
                    tangos_log.info(f"Triangulation root via query-NER: '{investigation_query}' -> subject '{subject}' -> '{root}'")
        if root:
            tangos_log.info(f"Triangulation root = '{root}' (investigation subject)")
        else:
        ...
        # chordal+planar maximally-filtered graph from `tangraph`, decomposing
        # it into (p-3) tetrahedra glued by triangular separators -- the cliques
        # are candidate investigation themes. Opt-in via TANGRAPH_TMFG=1; does
        # not replace `tangraph` or affect the standard pipeline. The result is
        # held for Phase-2 belief propagation (which fires after the consolidator
        # has computed per-entity `prob`).
        tmfg_result = None
        if os.environ.get("TANGRAPH_TMFG"):
            tmfg_result = construct_tmfg(tangraph)
            tangos_log.info(
                f"TMFG: {tangraph.number_of_edges()} -> {tmfg_result.graph.number_of_edges()} edges "
            ...
            for idx, members, w in ranked[:5]:
                tangos_log.info(f"  TMFG theme [{idx}] w={w:.2f}: {sorted(members)}")

        # Optional corroboration filter (Direction-1 research prototype). Drops
        # affiliation edges attested by fewer than TANGRAPH_EDGE_FILTER_MIN
        # chunks (default 2), preserving every node's path to root via
        # below-threshold bridges. Opt-in via env flag; not on by default.
        if os.environ.get("TANGRAPH_EDGE_FILTER"):
            min_count = int(os.environ.get("TANGRAPH_EDGE_FILTER_MIN", "2"))
            before = tangraph.number_of_edges()
            tangraph = filter_by_corroboration(tangraph, root, min_count=min_count)
            tangos_log.info(
                f"Corroboration filter: {before} -> {tangraph.number_of_edges()} edges "
                f"(min_count={min_count}, root={root})"
            )

        tangos_log.info("Starting node and evidence consolidation process")
        ...
        # Research mode: TANGRAPH_BP_KEEP_DROPPED keeps low-prob-evidenced
        ...
        merged_entities, edges_enrichment_results, _final_identifiers = score_graph_by_connectivity(
            ...
            root=root,
            keep_low_prob_evidenced=bool(os.environ.get("TANGRAPH_BP_KEEP_DROPPED")),
        )

        # Phase-2: junction-tree belief propagation over the TMFG. Runs only if
        # TMFG was built (TANGRAPH_TMFG=1). Fuses each entity's evidence prior
        ...
        if tmfg_result is not None:
            ...
            try:
                ...
                tangos_log.warning(f"Phase-2 BP failed: {type(e).__name__}: {e}")
        ...
        tangos_log.info(f"Time taken for graph generation: {time.time() - start_time:.1f}s")
        ...
        tangos_log.info(f"Graph state saved for session: {investigation_id}")
        tangos_log.info(f"Total RUNS for session: {investigation_id} is {run_number + 1}")
        tangos_log.info(
        ...
        # TANGRAPH_TMFG flag that gates Phase 1 + Phase 2. Backwards-compatible:
        ...
        if os.environ.get("TANGRAPH_TMFG"):
            try:
                ...
                tangos_log.warning(f"[Phase 3] network-analysis payload failed: {type(e).__name__}: {e}")
    ...
    def _build_network_analysis(self, response_nodes: list, response_edges: list) -> dict:
        ...
        # surface clean actor cliques). Set TANGRAPH_TMFG_UNIFORM_WEIGHTS=1 to
        # restore the old topology-only behaviour.
        evidence_aware = os.environ.get("TANGRAPH_TMFG_UNIFORM_WEIGHTS") != "1"
    ...
    def _render_phase2_visualizations(self, investigation_id, tmfg_result, bp_result,
        ...
        """
        if not (self.debug_mode or os.environ.get("TANGRAPH_VIZ")):
            return
        try:
            from tangraph.graph import _delta_color, _posterior_color, visualize_clique_tree
            viz_dir = os.environ.get("TANGRAPH_VIZ_DIR") or os.path.join(os.getcwd(), "debug_output", "viz")
            ...
            tangos_log.warning(f"[Phase 2] viz failed: {type(e).__name__}: {e}")
    ...
    def _render_debug_graph(self, investigation_id, entities, edges, query) -> None:
        ...
        Enabled by ``debug_mode`` or the ``TANGRAPH_VIZ`` env var. Nodes are sized
        ...
        try:
            ...
            tangos_log.info(f"[debug] triangulated-graph visualization written to {out_path}")
        except Exception as e:  # noqa: BLE001 — visualization must never break a run
            tangos_log.warning(f"[debug] graph visualization failed: {e}")
