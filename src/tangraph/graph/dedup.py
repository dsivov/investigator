...
def cluster_identifiers(identifiers: list[str]) -> list:
    if len(identifiers) < 200:
        tangos_log.debug("Not enough identifiers to cluster; returning single group.")
        return [identifiers]
    ...
    for i, label in enumerate(labels):
        ...
        tangos_log.debug(f"Identifier: {identifiers[i]}, Cluster Label: {label}")
    groups_lists = []
    for cluster_label, group in groups_dict.items():
        tangos_log.debug(f"Cluster {cluster_label}: {group}")
...
def merge_duplicate_group(dedup):
    ...
    tangos_log.debug(f"Merging {len(duplicates)} duplicates for record {dedup.record['identifier']}")
...
def deduplicate_entities(entities_dicts, representative_identifiers, semhash_model=None):
    ...
    tangos_log.debug(f"Total identifiers before deduplication: {len(all_identifiers)}")
    ...
    for entity in entities_dicts:
        ...
        if entity["identifier"].upper() in all_representative_ids:
            tangos_log.debug(f"Identifier {entity['identifier']} already mapped to a representative.")
            continue
    ...
    tangos_log.debug(f"Total mapped identifiers to representatives: {mapped_count}")
    if mapped_count != len(entities_dicts):
        tangos_log.debug("!!!!Some identifiers were not mapped to any representative.!!!!")
    tangos_log.debug(
    ...
    for dedup in deduplicated_records:
        if len(dedup.duplicates) < 1:
            tangos_log.debug(f"Skipping dedup for record {dedup.record['identifier']} without duplicates")
        ...
        all_deduplicated_identifiers.append(dedup.record["identifier"].upper())
        entity_groups.append(dedup.record)
    tangos_log.info(f"Total identifiers after deduplication: {len(all_deduplicated_identifiers)}")
    tangos_log.info(f"Total records after deduplication: {len(entity_groups)}")
