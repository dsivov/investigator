"""OSINTGraph configuration.

Wraps LightRAG's argparse-driven config and layers OSINTGraph-specific knobs on
top (``investigator_host``, ``investigator_port``, ``analytic_engine_enabled``, ...).

``global_args = parse_args()`` runs at import time — preserved for compatibility
with the existing call sites. Phase 3 candidate: replace argparse with
``pydantic_settings.BaseSettings`` so import-time side effects go away and the
module is safe to load under pytest / programmatic use.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    # OSINTGraph specific configurations can be added here
    parser.add_argument(
        "--analytic_engine_enabled",
        action="store_true",
        default=get_env_value("ANALYTIC_ENGINE_ENABLED", False, bool),
        help="Enable analytic engine (default: from env or False)",
    )

    parser.add_argument(
        "--investigator_host",
        default=get_env_value("INVESTIGATOR_HOST", "0.0.0.0"),
        help="Server investigator host (default: from env or 0.0.0.0)",
    )
    parser.add_argument(
        "--investigator_port",
        type=int,
        default=get_env_value("INVESTIGATOR_PORT", 5003, int),
        help="Server investigator port (default: from env or 5003)",
    )
    parser.add_argument(
        "--reranker_host",
        default=get_env_value("RERANKER_HOST", "0.0.0.0"),
        help="Server reranker host (default: from env or 0.0.0.0)",
    )
    parser.add_argument(
        "--reranker_port",
        type=int,
        default=get_env_value("RERANKER_PORT", 8182, int),
        help="Server reranker port (default: from env or 8182)",
    )
